<template>
  <el-dialog
    :title="type == 'add' ? 'Add goods' : 'Modify goods'"
    v-model="visible"
    width="400px"
    @close="handleClose"
  >
    <el-form :model="ruleForm" :rules="rules" ref="formRef" label-width="100px" class="good-form">
      ...
      <el-form-item label="Goods ID" prop="id">
  ...
  setup(props) {
    ...
    const state = reactive({
      ...
      rules: {
        ...
        id: [
          { required: 'true', message: 'ID cannot be empty', trigger: ['change'] }
        ],
        sort: [
          { required: 'true', message: 'Sort cannot be empty', trigger: ['change'] }
        ]
      },
      id: ''
    })
      ...
      formRef.value.validate((valid) => {
        if (valid) {
          ...
          if (state.ruleForm.sort < 0 || state.ruleForm.sort > 200) {
            ElMessage.error('Sort value cannot be less than 0 or greater than 200')
            return
          }
          if (props.type == 'add') {
