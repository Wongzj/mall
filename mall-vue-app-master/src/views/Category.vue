    ...
    <div>
      <header class="category-header wrap">
        ...
        <div class="header-search">
          ...
          <router-link tag="span" class="search-title" to="./product-list?from=category">On sale</router-link>
        </div>
                ...
                <template v-for="(category, index) in categoryData">
                  <div class="swiper-slide" v-if="currentIndex == category.id" :key="index">
                    ...
                    <!-- <div class="category-list" v-for="(products, index) in category.secondLevelCategoryVOS" :key="index">
                      <p class="catogory-title">{{products.name}}</p> -->
                      <p class="catogory-title">{{index}}</p>
                      <div class="product-item" v-for="(category, index) in category.children" :key="index" @click="selectProduct(category)">
                        <img src="//s.weituibao.com/1583591077131/%E5%88%86%E7%B1%BB.png" class="product-img"/>
                        <p v-text="category.name" class="product-title"></p>
                      </div>
                    <!-- </div> -->
                  </div>
                </template>
...
export default {
  ...
  methods: {
    ...
    selectProduct(item){
      this.$router.push({ path: `product-list?id=${item.id}` })
    },
  }
}
</script>
