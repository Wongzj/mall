...
<template>
  <div class="create-order">
    ...
    <div class="pay-wrap">
      <div class="price">
        <span>The amount of goods</span>
        <span>¥{{ total }}</span>
      </div>
      <van-button @click="createOrder" class="pay-btn" color="#1baeae" type="primary" block>Generate orders</van-button>
    </div>
    <van-popup
      closeable
    ...
    >
      <div :style="{ width: '90%', margin: '0 auto', padding: '50px 0' }">
        <van-button :style="{ marginBottom: '10px' }" color="#1989fa" block @click="payOrder(1)">AliPay</van-button>
        <van-button color="#4fc08d" block @click="payOrder(2)">WeChat Pay</van-button>
      </div>
    </van-popup>
  </div>
</template>
  ...
  methods: {
    ...
    async createOrder() {
      const params = {
        ...
        cartItemIds: this.cartList.map(item => item.id)
      }
